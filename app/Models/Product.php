                ...
                if (in_array(0, $exist)) {
                    $query->orWhereRaw(
                        DB::raw("((CASE WHEN (products.product_quantity - (SELECT SUM(order_details.quantity) FROM orders INNER JOIN order_details ON orders.id = order_details.order_id WHERE order_details.product_id = products.id AND orders.order_status IN ('1', '2') GROUP BY order_details.product_id)) IS NOT NULL THEN
                            (products.product_quantity - (SELECT SUM(order_details.quantity) FROM orders INNER JOIN order_details ON orders.id = order_details.order_id WHERE order_details.product_id = products.id AND orders.order_status IN ('1', '2') GROUP BY order_details.product_id)) ELSE products.product_quantity END)) = 0")
                    );
                }
                if (in_array(1, $exist)) {
                    $query->orWhereRaw(
